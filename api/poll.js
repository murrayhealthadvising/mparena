...
function fetchNewLeads(addr, pass, seenIds) {
            ...
            msg.once('end', async () => {
              try {
                ...
                if (lead.firstName || lead.phone) {
                  lead.emailMsgId = parsed.messageId || '';
                  results.push(lead);
                }
              } catch (e) {}
            });
...
module.exports = async (req, res) => {
  ...
  const { addr, pass, seenIds = [] } = req.body;
  ...
  try {
    const leads = await fetchNewLeads(addr, pass, seenIds);
    res.status(200).json({ leads });
  } catch (err) {
    console.error('IMAP error:', err.message);
    res.status(500).json({ error: 'Failed to connect to Gmail', detail: err.message });
  }
};
