...
function dlNoteSave(){
  ...
  bktNoteTimers['dl']=setTimeout(function(){
    ...
    // sync to card textarea
    var ta=document.getElementById('bkt-note-'+l.id);
    if(ta)ta.value=l.notes;
  },800);
}
var TC={new:'color:#94a3b8;background:#1a2133',callback:'color:#85B7EB;background:#042C53',noanswer:'color:#EF9F27;background:#2a1f08',missedapt:'color:#ED93B1;background:#2a0f1c',pr:'color:#5DCAA5;background:#04342C',quoted:'color:#AFA9EC;background:#1a1540',ghost:'color:#F0997B;background:#2d1508'};
var TL={new:'New',callback:'Call back',noanswer:'No answer',missedapt:'Missed apt',pr:'PR ✓',quoted:'Quoted',ghost:'Ghost'};
function renderBkt(){
  ...
  if(!bl.length){list.innerHTML='<div class="bkt-empty">No high priority leads in the last 7 days.<br><span style="font-size:12px;color:#475569">Leads with New, Call back, No answer, or Missed apt dispositions from the last 7 days appear here.</span></div>';return;}
  var rows=[];
...
function dialerShow(){
  ...
  // Load notes into dialer notes field
  var dlNotes=document.getElementById('dl-notes');
  if(dlNotes){dlNotes.value=l.notes||'';dlNotes.placeholder='Notes for '+( l.firstName||'this lead')+'...';}
  var card=document.getElementById('pc-'+l.id);if(card)card.scrollIntoView({behavior:'smooth',block:'center'});
  renderBkt();
}
function dialerNext(){dialerIdx++;if(dialerIdx>=dialerQueue.length){stopDialer();return;}dialerShow();}
function dialerDisp(val){
  if(!dialerQueue.length||dialerIdx>=dialerQueue.length)return;
  var l=dialerQueue[dialerIdx];l.disposition=val;
  if(typeof updateStats==='function')updateStats();
  if(typeof updateField==='function')updateField(l.id,{disposition:val});
  dialerNext();
}
function stopDialer(){
  dialerActive=false;dialerQueue=[];dialerIdx=0;
